...
static volatile uint32_t systemTicks = 0;  // Global tick counter
...
// Initialize Timer 0 for 10ms intervals
void initTimer0(void)
{
    // Configure Timer 0 in 16-bit mode
    TMOD = (TMOD & ~bT0_M1) | bT0_M0;  // Mode 1: 16-bit timer
    
    // Calculate reload value for 10ms
    // System clock = 48MHz
    // Timer clock = System clock / 12 = 4MHz
    // For 10ms, we need 40000 counts
    // 65536 - 40000 = 25536 (initial value)
    TH0 = (25536 >> 8) & 0xFF;
    TL0 = 25536 & 0xFF;
...
{
    return (systemTicks - startTick) * 10;
}